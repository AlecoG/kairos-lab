...
const BTN_BASE = "inline-flex min-h-[42px] cursor-pointer items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-[0.84rem] font-semibold transition-[background-color,border-color,box-shadow,color,transform] duration-200 ease-out focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-black/6 active:scale-[0.99]";
const BTN_PRIMARY = `${BTN_BASE} border-[#1a1a1a] bg-[#1a1a1a] text-white shadow-[0_1px_2px_rgba(0,0,0,0.08)] hover:bg-[#111111] hover:border-[#111111]`;
const BTN_GHOST = `${BTN_BASE} border-[#dddddd] bg-white text-[#111111] shadow-[0_1px_2px_rgba(0,0,0,0.03)] hover:border-[#cfcfcf] hover:bg-[#fafafa]`;
const BTN_CARD = "inline-flex h-[34px] items-center justify-center rounded-[8px] border border-transparent bg-[#f1f2f5] px-4 text-[12.5px] font-semibold text-[#161616] shadow-none outline-none transition-colors duration-150 hover:bg-[#ebedf1] focus-visible:ring-0";
...
function normalizeProductShape(product) {
  const stockStatus = product?.stock_status || product?.stockStatus || "disponible";
...
function getFallbackProducts() {
  ...
  const visible = products.filter((p) => p.visible !== false && p.enabled !== false);
  const normalized = visible.map(normalizeProductShape);
  return isCatalogPage ? normalized : normalized.slice(0, 3);
}
...
async function loadStorefrontProducts() {
  try {
    ...
    const normalized = data.map(normalizeProductShape);
    storefrontProducts = isCatalogPage ? normalized : normalized.slice(0, 3);
  } catch (error) {
    console.warn("No se pudo cargar catálogo desde API, usando fallback local.", error);
    storefrontProducts = getFallbackProducts();
  }
}
...
function renderServices() {
  ...
  grid.innerHTML = services.map((s) => `
    <article class="group grid min-h-[208px] gap-4 rounded-[26px] border border-[#e6e6e6] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,248,248,0.98))] p-5 shadow-[0_10px_30px_rgba(0,0,0,.05)] transition-all duration-300 hover:-translate-y-1 hover:border-[#d2d2d2] hover:shadow-[0_22px_46px_rgba(0,0,0,.10)]">
      <div class="flex items-start justify-between gap-4">
        <div class="grid gap-2">
          <span class="inline-flex w-fit rounded-full border border-[#ececec] bg-white px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-[0.18em] text-[#8a8a8a]">Servicio</span>
          <h3 class="m-0 text-[1.2rem] font-black tracking-[-0.02em] text-[#111111]">${s.name}</h3>
        </div>
        <span class="whitespace-nowrap rounded-full border border-[#dddddd] bg-white px-3 py-1.5 text-[0.78rem] font-extrabold uppercase tracking-[0.08em] text-[#5f5f5f] shadow-[0_6px_16px_rgba(0,0,0,.04)]">${s.minutes} min</span>
      </div>
      <p class="m-0 max-w-[34ch] text-[0.98rem] leading-relaxed text-[#666666]">${s.desc}</p>
      <div class="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-[#ececec] pt-4">
        <span class="text-[1.05rem] font-black tracking-[-0.02em] text-[#111111]">${s.price}</span>
        <button class="${BTN_CARD} min-w-[140px]" data-service="${s.name}">Agendar este servicio</button>
      </div>
    </article>
  `).join("");
...
function renderProducts(list) {
  ...
  grid.innerHTML = list.map((p) => {
    ...
    const statusLabel = isOutOfStock
      ? `<span class="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-[0.72rem] font-extrabold uppercase tracking-[0.12em] text-red-700">Agotado</span>`
      : "";
    return `
    <article class="group grid min-h-[100%] gap-4 rounded-[28px] border border-[#e6e6e6] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(247,247,247,0.98))] p-4 shadow-[0_10px_30px_rgba(0,0,0,.05)] transition-all duration-300 hover:-translate-y-1 hover:border-[#d4d4d4] hover:shadow-[0_22px_46px_rgba(0,0,0,.10)] max-[640px]:[flex:0_0_84%]">
      <div class="overflow-hidden rounded-[22px] border border-[#ececec] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.98),rgba(239,239,239,0.96))] p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
        <img
          class="js-product-image block h-[310px] w-full origin-center object-contain transition-transform duration-500 ease-out group-hover:scale-[1.04]"
      ...
      <div class="grid gap-3">
        <div class="flex items-start justify-between gap-4">
          <div class="grid gap-2">
            <span class="inline-flex w-fit rounded-full border border-[#ececec] bg-white px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-[0.18em] text-[#8a8a8a]">${p.category}</span>
            <h3 class="m-0 text-[1.18rem] font-black tracking-[-0.02em] text-[#111111]">${p.name}</h3>
          </div>
          ${statusLabel}
        </div>
        <p class="m-0 text-[0.98rem] leading-relaxed text-[#666666]">${p.desc}</p>
        <div class="mt-auto grid gap-3 border-t border-[#ececec] pt-4">
          <div class="flex items-center justify-between gap-3">
            <span class="text-[1.08rem] font-black tracking-[-0.02em] text-[#111111]">${p.price}</span>
            <span class="text-[0.72rem] font-bold uppercase tracking-[0.16em] text-[#9a9a9a]">Kairos Lab</span>
          </div>
          <div class="flex flex-wrap gap-2.5">
            ${isCatalogPage ? `<button class="${BTN_CARD} min-w-[140px] disabled:cursor-not-allowed disabled:opacity-50" data-add-product-id="${p.id}" ${isOutOfStock ? "disabled" : ""}>${isOutOfStock ? "No disponible" : "Agregar al carrito"}</button>` : ""}
            <button class="${BTN_CARD} min-w-[140px]" data-product-id="${p.id}">Preguntar por este producto</button>
          </div>
        </div>
      </div>
    </article>
...
function initSearch() {
    ...
    cartItems.innerHTML = Array.from(cartState.values()).map((item) => `
      <div class="flex items-center justify-between gap-3 rounded-xl border border-[#e8e8e8] bg-white px-3 py-2.5 max-[640px]:flex-col max-[640px]:items-start">
        ...
        <div class="flex items-center gap-2">
          <button class="min-w-[34px] cursor-pointer rounded-[10px] border border-[#e8e8e8] bg-white px-2 py-1 font-extrabold" type="button" data-cart-action="decrease" data-cart-product-id="${item.id}" aria-label="Quitar una unidad">-</button>
          <span class="min-w-[20px] text-center font-extrabold">${item.qty}</span>
          <button class="min-w-[34px] cursor-pointer rounded-[10px] border border-[#e8e8e8] bg-white px-2 py-1 font-extrabold" type="button" data-cart-action="increase" data-cart-product-id="${item.id}" aria-label="Agregar una unidad">+</button>
          <button class="cursor-pointer rounded-[10px] border border-[#e8e8e8] bg-white px-2 py-1 text-sm font-extrabold text-[#6b6b6b]" type="button" data-cart-action="remove" data-cart-product-id="${item.id}">Quitar</button>
        </div>
      </div>
    `).join("");
  ...
  const updateCartItem = (productId, action) => {
    ...
    if (action === "increase") {
      ...
      current.qty -= 1;
      if (current.qty <= 0) cartState.delete(productId);
  ...
  cartItems?.addEventListener("click", (e) => {
    ...
    const productId = actionBtn.getAttribute("data-cart-product-id") || "";
